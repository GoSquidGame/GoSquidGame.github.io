...
const main = async () => {
    ...
    clog('<< TEST in NFT contract, not staking contract >>');
    ...
    // clog('<staker can approve>')
    // let tokenId = await nftInstance.tokenOfOwnerByIndex(a1.address, prevNftBalance - 1);
    // let maticPredicateAddress = erc721PredicateAddress; //await nftInstance.bridgeAddr();
    // // clog(maticPredicateAddress);
    // clog(tokenId.toNumber());
    // clog(` - a1 address is ${a1.address}`);
    // clog(` - staked token's owner is ${await nftInstance.ownerOf(tokenId)}`);
    // // approve
    // let tx = await nftInstance.connect(a1).approve(maticPredicateAddress, tokenId);
    // await tx.wait();


    clog('<staker can do depositFor>')
    // depositFor for deposit into L2

    clog(` - getApproved: ${await nftInstance.getApproved(hre.ethers.BigNumber.from(58))}`)
    const abiCoder = new hre.ethers.utils.AbiCoder();
    const depositData = abiCoder.encode(['uint256'], [hre.ethers.BigNumber.from(58)]);
